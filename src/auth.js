...
export const { auth } = CreateAuth({
  ...
  cookies: {
    sessionToken: {
      ...
      options: {
        ...
        domain: process.env.AUTH_URL?.includes("onereel.online")
          ? ".onereel.online"
          : undefined,
      },
    },
  ...
  csrf: true,
  debug: false, // ✅ Disabled — was generating 100+ logs per request in production

  callbacks: {
    async session({ session, user, token }) {
      const effectiveUserId = user?.id || token?.sub;
      if (session?.user) {
